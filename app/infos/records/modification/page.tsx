        ...
        <main className="container mx-auto px-4 py-12 pt-32 text-slate-900">
          ...
          <div className="flex items-center gap-4 mb-12">
            <Link href="/infos/records"
                  className="p-3 bg-white hover:bg-slate-100 rounded-2xl transition-colors text-slate-400 shadow-sm border border-slate-200">
              <ArrowLeft size={24}/>
            </Link>
              ...
              <div key={section.title} className="mb-20">
                ...
                <div
                  ...
                  <table className="w-full text-left border-collapse table-fixed">
                    <thead
                        className="bg-slate-900 text-[10px] font-black uppercase tracking-widest text-slate-400">
                    <tr>
                        ...
                        <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                          ...
                          <td colSpan={3} className="p-0">
                            <form action={updateRow}
                                  className="grid grid-cols-[1fr_1fr_80px] h-full items-center">
                              <input type="hidden" name="id" value={r.id}/>

                              {/* OUTDOOR */}
                              <div className="p-4 flex flex-col gap-2 border-l border-slate-100">
                                <input name="out_athlete" defaultValue={r.out_athlete || ""}
                                       placeholder="Athlète"
                                       className="w-full px-3 py-1.5 bg-slate-100 border border-slate-200 focus:border-red-600 focus:bg-white outline-none font-bold rounded-lg text-slate-900"/>
                                <div className="flex gap-2">
                                  <input name="out_perf" defaultValue={r.out_perf || ""}
                                         placeholder="Perf"
                                         className="w-2/3 px-3 py-1.5 bg-slate-100 border border-slate-200 focus:border-red-600 focus:bg-white outline-none font-black italic text-red-600 rounded-lg"/>
                                  <input name="out_year" defaultValue={r.out_year || ""}
                                         placeholder="Année"
                                         className="w-1/3 px-3 py-1.5 bg-slate-100 border border-slate-200 focus:border-red-600 focus:bg-white outline-none text-slate-500 rounded-lg text-center"/>
                                </div>
                              </div>

                              {/* INDOOR */}
                              <div className="p-4 flex flex-col gap-2 border-l border-slate-100">
                                <input name="in_athlete" defaultValue={r.in_athlete || ""}
                                       placeholder="Athlète"
                                       className="w-full px-3 py-1.5 bg-slate-100 border border-slate-200 focus:border-slate-900 focus:bg-white outline-none font-bold rounded-lg text-slate-900"/>
                                <div className="flex gap-2">
                                  <input name="in_perf" defaultValue={r.in_perf || ""}
                                         placeholder="Perf"
                                         className="w-2/3 px-3 py-1.5 bg-slate-100 border border-slate-200 focus:border-slate-900 focus:bg-white outline-none font-black italic text-slate-900 rounded-lg"/>
                                  <input name="in_year" defaultValue={r.in_year || ""}
                                         placeholder="Année"
                                         className="w-1/3 px-3 py-1.5 bg-slate-100 border border-slate-200 focus:border-slate-900 focus:bg-white outline-none text-slate-500 rounded-lg text-center"/>
                                </div>
                              </div>
                              ...
                              <div className="flex justify-center border-l border-slate-100">
                                <button type="submit"
                                        className="p-3 bg-slate-100 text-slate-400 rounded-xl hover:bg-green-600 hover:text-white transition-all shadow-sm">
                                  <Save size={20}/>
                                </button>
                              </div>
                            </form>
