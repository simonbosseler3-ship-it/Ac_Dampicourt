        ...
        <main className="container mx-auto px-4 py-12 pt-32">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-4">
            ...
            {isAdmin && (
                <Link href="/club/modifier">
                  <button className="flex items-center gap-2 bg-slate-900 text-white px-6 py-2 rounded-full font-bold hover:bg-red-600 transition-all text-sm uppercase italic">
          ...
          <div className="mb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <p className="text-slate-600 text-xs font-bold leading-relaxed">Piste synthétique 6 couloirs en réfection : passage à 8 couloirs.</p>
              </div>
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                <h3 className="text-red-600 font-black text-xs uppercase italic mb-2 tracking-tighter">Sautoirs</h3>
                <p className="text-slate-600 text-xs font-bold leading-relaxed">Sautoir en longueur et piste de survitesse en Mondo (2010).</p>
              </div>
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                <h3 className="text-red-600 font-black text-xs uppercase italic mb-2 tracking-tighter">Endurance</h3>
                <p className="text-slate-600 text-xs font-bold leading-relaxed">Circuit de copeaux de 1 km pour l'entraînement (2006).</p>
              </div>
              <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                <h3 className="text-red-600 font-black text-xs uppercase italic mb-2 tracking-tighter">Perche</h3>
                <p className="text-slate-600 text-xs font-bold leading-relaxed">Portique multifonctionnel pour l'entraînement (2010).</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
              {comite?.map((member) => (
                  <div key={member.id} className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
            ...
            <div className="bg-slate-50 rounded-3xl p-8 flex flex-col md:flex-row justify-between items-center gap-6 border border-slate-200">
              ...
              <div className="text-right">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Coordinateur Sportif & Technique</span>
                <span className="text-lg font-black italic uppercase text-slate-800">{info?.sport_coordinator}</span>
              </div>
            </div>
          </div>
        </main>
