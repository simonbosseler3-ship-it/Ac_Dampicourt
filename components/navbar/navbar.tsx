...
export function Navbar() {
  ...
  // Fonction pour changer le thème globalement en DB
  const handleThemeChange = async (newTheme: string) => {
    // 1. Mise à jour immédiate de l'interface (UI)
    setTheme(newTheme);

    // 2. Sauvegarde en Base de données pour que ce soit persistant pour tous
    // Note : Adapte 'site_settings' et la colonne selon le nom de ta table en DB
    ...
    if (error) {
      console.error("Erreur lors de la mise à jour du thème en DB:", error.message);
    }
  };
      ...
      <>
        <header className="fixed top-0 left-0 right-0 z-[100] w-full border-b-4 border-red-600 bg-white/80 backdrop-blur-md shadow-sm h-16 md:h-20">
          <div className="container mx-auto flex h-full items-center justify-between px-4">
            ...
            {/* AUTH & THEME */}
            <div className="flex items-center gap-2 lg:gap-4 shrink-0 min-w-[150px] justify-end">

              {/* SWITCHER DE THÈME - RÉSERVÉ ADMIN */}
              {profile?.role === 'admin' && (
                  <div className="flex items-center gap-1 bg-slate-100/50 p-1 rounded-xl border border-slate-200">
                    <button
                        onClick={() => handleThemeChange("default")}
                        className={`p-1.5 rounded-lg transition-all ${theme === 'default' ? 'bg-white shadow-sm text-red-600' : 'text-slate-400 hover:text-slate-600'}`}
                    ...
                    <button
                        ...
                        className={`p-1.5 rounded-lg transition-all ${theme === 'christmas' ? 'bg-white shadow-sm text-green-600' : 'text-slate-400 hover:text-green-500'}`}
                        title="Thème Noël"
                    >
                      <TreePine size={16} />
                    </button>

                    {/* BOUTON PÂQUES À RAJOUTER ICI */}
                    <button
                        onClick={() => handleThemeChange("easter")}
                        className={`p-1.5 rounded-lg transition-all ${theme === 'easter' ? 'bg-white shadow-sm text-pink-500' : 'text-slate-400 hover:text-pink-400'}`}
                        title="Thème Pâques"
                    >
                      <Egg size={16} />
                    </button>
              ...
              {!loading ? (
                  user ? (
                      ...
                      <Link href="/login" prefetch={false}>
                        <Button className="bg-red-600 hover:bg-red-700 text-white font-black rounded-xl px-4 lg:px-6 h-10 shadow-lg shadow-red-200 uppercase italic text-[10px] lg:text-xs transition-all active:scale-95 whitespace-nowrap">Connexion</Button>
                      </Link>
                  )
              ) : (
        ...
        {/* MOBILE MENU */}
        <div className={`fixed inset-0 bg-black/50 z-[120] transition-opacity duration-300 md:hidden ${isMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`} onClick={() => setIsMenuOpen(false)} />
        <div className={`fixed top-0 left-0 h-full w-[300px] bg-white z-[130] shadow-2xl transition-transform duration-300 md:hidden ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2 font-black uppercase italic text-sm">
              ...
              <button onClick={() => setIsMobileInfosOpen(!isMobileInfosOpen)} className="w-full flex items-center justify-between p-3 font-black uppercase italic">
                Infos <ChevronDown size={16} className={isMobileInfosOpen ? "rotate-180 text-red-600" : ""} />
              </button>
              {isMobileInfosOpen && (
                  <div className="bg-slate-50 rounded-lg flex flex-col pl-4 text-[11px]">
                    ...
                    <Link href="/infos/ethique" prefetch={false} className="p-3">Éthique & Dopage</Link>
                  </div>
              )}
